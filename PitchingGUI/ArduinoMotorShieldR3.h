...
class ArduinoMotorShieldR3
{
  public:
    ...
    // PUBLIC METHODS WHEN YOU ADD 2 AFTER FUNCTION, JUST MEANS YOUR SETTING BOTH
    void init(); // Set pin directions
    void setSpeed(unsigned char motor, float speed); // Set speed for motor.
    void setSpeed2(float m1Speed, float m2Speed); // Set speed for both M1 and M2. for 2 wheels
  ...
  private:
    unsigned char DIR_A;//DIRECTION 
    unsigned char BRK_A;//BREAK
    unsigned char PWM_A;//VALUE OF PULSE/SPEED OF MOTOR
    unsigned char CS_A;//CURRENT SENSING
